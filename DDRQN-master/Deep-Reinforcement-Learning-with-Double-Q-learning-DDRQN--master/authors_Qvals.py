...
fname_formats = ("auth-dtc-{}.csv", "auth-dtc-q-{}.csv")
results_path = "results/auth-dtc/"
...
if len(argv) > 1:
    ...
    score_file, q_file = \
            open(results_path + 'auth-dtc.csv', 'w+'), \
            open(results_path + 'auth-dtc-q.csv', 'w+')
...
def learn_from_memory():
    ...
    if memory.size > batch_size:
        ...
        get_q_zeros()
        q2 = np.max(get_q_values(s2), axis=1)[4:sample_size]

        get_q_zeros()
        target_q = get_q_values(s1)[4:sample_size]

        target_q[np.arange(target_q.shape[0]), a[4:sample_size]] = r[4:sample_size] + (1 - isterminal[4:sample_size]) * (discount_factor * q2)
...
if __name__ == '__main__':
    ...
    for _ in range(episodes_to_watch):
        ...
        kills = game.get_game_variable(GameVariable.KILLCOUNT)
        total_kills += float(kills)
        deaths = float(game.get_game_variable(GameVariable.DEATHCOUNT))
        ammo = game.get_game_variable(GameVariable.SELECTED_WEAPON_AMMO)
        print("Amount of ammo used: ", 26 - ammo)
        print("Number of kills: ", kills)
        print("Accuracy: ", round(kills/(26 - ammo), 2))
        # Sleep between episodes
        sleep(1.0)
        score = game.get_total_reward()
        print("Total score: ", score)
        print("Total number of deaths: ", deaths)
        print("K/D ratio: ", round(total_kills/deaths,2))
