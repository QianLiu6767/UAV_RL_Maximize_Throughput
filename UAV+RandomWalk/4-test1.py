# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/9/23

# DQN + 高斯马尔科夫移动模型 + 用户关联 + 线下线上
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DQN-GMR+用户关联')
...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        # 判断模型保存路径是否存在，不存在就创建
        if not os.path.exists('DDQN_tmp/'):
            os.mkdir('DDQN_tmp/')
        ...
        with tf.Session() as sess:
            ...
            if os.path.exists('DDQN_tmp/checkpoint'):                # 判断模型是否存在
                saver.restore(sess, 'DDQN_tmp/model.ckpt')           # 存在就从模型中恢复变量
            else:
                ...
                for step in range(100000):
                    ...
                    if step % 1000 == 0:
                        ...
                        save_path = saver.save(sess, "DDQN_tmp/model.ckpt")
