# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/10/12

# DQN + 高斯马尔科夫移动模型 + 用户关联 + 用户角度随机，移动模型中有高斯变量（初始位置变化)  + 神经网络层数变化
...
V = [10, 10, 10, 10, 10, 15, 15, 15, 15, 15, 20, 20, 20, 20, 20]                # 人群移动初始速度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DQN-GMR+用户关联+随机变量-3thNN')
    ...
    def step(self, observation, action, t):
        ...
        # 飞行能耗
        ef = p * (np.sqrt((s[0, 0] - next_coords[0][0]) ** 2 + (s[0, 1] - next_coords[0][1]) ** 2)) / v

        # 盘旋能耗
        distance = (next_coords[0][0] - self.user_center[o, 0])**2 + (next_coords[0][1] - self.user_center[o, 1])**2

        eh = p * 5 * (u[o] / np.emath.log2(1 + ((p*(np.sqrt(100**2 + distance)**(-4)))/sigma)))
        ...
        # --------------------------------------下一个状态------------------------------------------
        self.user_center[:] = self.update_env(V, W, t)                  # 更新用户位置,速度,角度
...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            # eval_net 的第三层. collections 是在更新 target_net 参数时会用到
            with tf.variable_scope('l3'):
                w3 = tf.get_variable('w2', [self.n_actions, self.n_actions], initializer=w_initializer, collections=c_names)
        ...
        with tf.variable_scope('target_net'):
            ...
            # target_net 的第三层. collections 是在更新 target_net 参数时会用到
            with tf.variable_scope('l3'):
                w3 = tf.get_variable('w2', [self.n_actions, self.n_actions], initializer=w_initializer, collections=c_names)
                b3 = tf.get_variable('b2', [1, self.n_actions], initializer=b_initializer, collections=c_names)
...
def run_maze():
    ...
    plt.plot(x1, g1, marker='o', markersize=10)
    ...
    for i in range(len(average_return1)):
        if i % 100 == 0:
            x2.append(np.arange(len(average_return1))[i])
            r1.append(average_return1[i])

    plt.plot(x2, r1, marker='o', markersize=10)
