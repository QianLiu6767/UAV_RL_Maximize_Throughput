# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/9/30

# DQN + 高斯马尔科夫移动模型 + 用户关联 + 用户角度随机，移动模型中有高斯变量（初始位置变化）+ H=100
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DQN-GMR+用户关联+随机变量')
...
def run_maze():
    ...
    for i in range(len(episode_return1)):
        if i % 1000 == 0:
            x3.append(np.arange(len(episode_return1))[i])
            r.append(episode_return1[i])

    plt.plot(x3, r, markersize=10)
    ...
    for i in range(len(episode_migration1)):
        if i % 1000 == 0:
            x4.append(np.arange(len(episode_migration1))[i])
            y4.append(episode_migration1[i])
    ...
    plt.ylabel('Episode_Throughput', font2)
    plt.xlabel('Training episodes', font2)
    plt.show()
