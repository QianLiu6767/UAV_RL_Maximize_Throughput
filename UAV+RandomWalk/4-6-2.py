...
# DDQN + 高斯马尔科夫移动模型 + 用户关联（A1, A1）15*25个动作
...
W = [30, 45, 60, 90, 120, 135, 150, 180, 210, 225, 240, 270, 300, 330, 360]     # 人群移动初始角度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DDQN-GMR+用户关联')
...
def run_maze():
    ...
    plt.savefig('a_t_DDQN_GMR_USER_100000.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Training Episodes', font2)
    plt.savefig('a_r_DDQN_GMR_USER_1000000.eps', bbox_inches='tight')
    plt.show()
