# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/9/12

# Double Q_learning + 高斯马尔科夫移动模型
# 100000次不收敛
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.battery = 10
        self.title('UAV测试环境-Double-Q_Learning-GMR')
...
class Dream:
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:                  # choose best action
            ...
            state_action1 = state_action1.reindex(np.random.permutation(state_action1.index))     # np.random.permutation: 随机排列一个序列，返回一个排列的序列

            state_action2 = self.Q_table.loc[observation, :]

            state_action2 = state_action2.reindex(np.random.permutation(state_action2.index))     # some actions have same value
    ...
    def learn(self, s, a, r, s_):
        ...
        if np.random.uniform() < 0.5:                                      # 更新q表
            ...
            q_target = r + self.gamma * self.Q_table.ix[s_, :].max() - self.q_table.ix[s, a]       # next state is not terminal，真实值                                              # next state is terminal，真实值
...
def update():
    ...
    plt.savefig('DQ_a_r_20000.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.xlabel('Training Episodes', font2)
    plt.savefig('DQ_a_t_20000.eps', bbox_inches='tight')
    plt.show()
