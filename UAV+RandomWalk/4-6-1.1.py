# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/10/9

# DQN + 高斯马尔科夫移动模型 + 用户关联 + 用户角度随机，移动模型中有高斯变量（初始位置变化）                   + H=50
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DQN-GMR+用户关联+随机变量-50m')
    ...
    def step(self, observation, action, t):
        ...
        eh = p * 5 * (u[o] / np.emath.log2(1 + ((p*(np.sqrt(50**2 + distance)**(-4)))/sigma)))
