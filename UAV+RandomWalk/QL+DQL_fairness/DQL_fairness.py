...
# Double Q_learning + 高斯马尔科夫移动模型 + 用户关联
# 回合数: 2395   内存不足
# Q_Table尺寸 (52831, 375)
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.battery = 200
        self.title('UAV测试环境-Double-Q-50m')
...
class Dream:
    ...
    def check1_state_exist(self, state):
    ...
    def check2_state_exist(self, state):
        if state not in self.Q_table.index:
            # append new state to q table，其实也就是初始化为0
            self.Q_table = self.Q_table.append(
                pd.Series(
    ...
    def choose_action(self, observation, U):
        ...
        self.check1_state_exist(observation)       # 检验observation是否在q表
        self.check2_state_exist(observation)       # 检验observation是否在Q表
        ...
        while flag:
            ...
            if np.random.uniform() < self.epsilon:    # choose best action
                state_action1 = self.q_table.loc[observation, :]
                state_action1 = state_action1.reindex(
                    np.random.permutation(state_action1.index))  # np.random.permutation: 随机排列一个序列，返回一个排列的序列
                state_action2 = self.Q_table.loc[observation, :]
                state_action2 = state_action2.reindex(
                    np.random.permutation(state_action2.index))  # some actions have same value
                if state_action1.max() >= state_action2.max():
                    action = state_action1.idxmax()
                else:
                    action = state_action2.idxmax()
    ...
    def learn(self, s, a, r, s_):
        ...
        self.check1_state_exist(s_)
        self.check2_state_exist(s_)

        if np.random.uniform() < 0.5:                                      # 更新q表

            # loc——通过行标签索引行数据
            # iloc——通过行号索引行数据
            # ix——通过行标签或者行号索引行数据（基于loc和iloc 的混合）

            q_predict = self.q_table.ix[s, a]                              # 估计值, 另一种：q_predict = self.q_table.loc[s, a]

            q_target = r + self.gamma * self.Q_table.ix[s_, :].max() - self.q_table.ix[s, a]       # next state is not terminal，真实值

            self.q_table.ix[s, a] = (1 - self.lr) * q_predict + self.lr * q_target                 # update q表

            return self.q_table, self.Q_table
        else:                                                              # 更新Q表
            ...
            self.Q_table.loc[s, a] += self.lr * (q_target - q_predict)     # update Q表
...
def update():
    ...
    for episode in range(2000):
        ...
        while True:
            ...
            action, Flag = RL.choose_action(str(observation), U)

            observation_, reward, Reward, done, u = env.step(observation, action, Flag, T, U)  # tau为step计数器
            ...
            q_table, Q_table = RL.learn(str(observation), action, reward, str(observation_))
    ...
    plt.plot(x1, r1, marker='o', label='DQL', markersize=10)
    ...
    plt.savefig('a_r_DQL_2000_50m.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.plot(x1, g1, marker='o', label='DQL', markersize=10)
    ...
    plt.xlabel('Training Episodes', font2)
    plt.savefig('a_t_DQL_2000_50m.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.plot(x1, y1, marker='o', label='DQL', markersize=10)
    ...
    plt.xlabel('Training Episodes', font2)
    plt.savefig('E_t_DQL_2000_50m.eps', bbox_inches='tight')
    plt.show()
