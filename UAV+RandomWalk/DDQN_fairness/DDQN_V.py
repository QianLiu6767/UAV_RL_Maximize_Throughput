# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/11/15

# DDQN + 高斯马尔科夫移动模型 + 用户关联（A1, A1）15*25个动作 + 用户角度随机，移动模型中有高斯变量（初始位置变化）
# 用户公平性 + 约束最后一个点尽可能的近一点 + 大奖励作为性能输出
# 改变参数 无人机速度v=16.7 对应-Fc=[1.0,...,3.0]
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DDQN-50m-改变v=16.7-Fc=[1.0-3.0]')
    ...
    def step(self, observation, action, FLAG, t, U, F):
...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        #     save_path = saver.save(sess, "DDQN_50m/save_net.ckpt", global_step=N)
...
def run_maze():
    for F_c in range(10, 31, 2):
        ...
        for episode in range(N):
            ...
            while True:
                ...
                observation_, reward, Reward, done, u = env.step(observation, action, Flag, T, U, F_c)  # 状态更新，包括用户移动更新
                ...
                if done:
                    ...
                    episode_step1.append(T)  # 回合step数
                    ...
                    break
        print("CPU计算频率：", F_c)
        ET.append(np.mean(episode_migration1))
        STEP.append(np.mean(episode_step1))

    print("CPU计算频率Fc对应的每回合step数", STEP)
    print("CPU计算频率Fc对应的回合吞吐量", ET)

    # Fc对应的回合吞吐量图
    plt.figure(figsize=(10, 8))
    x1 = [1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4, 2.6, 2.8, 3.0]
    y1 = []

    for i in range(11):
        y1.append(ET[i])

    plt.plot(x1, y1, marker='o', label='DDQN-Fc=(1,...,3)', markersize=10)
    ...
    plt.ylabel('Episode Throughput', font2)
    plt.xlabel('CPU Frequency', font2)
    plt.savefig('E_t_DDQN.eps', bbox_inches='tight')
    plt.show()
    ...
    plt.plot(x1, y2, marker='o', label='DDQN-Fc=(1,...,3)', markersize=10)
    ...
    plt.ylabel('Episode Steps', font2)
    plt.xlabel('CPU Frequency', font2)
    plt.savefig('E_s_DDQN.eps', bbox_inches='tight')
    plt.show()
