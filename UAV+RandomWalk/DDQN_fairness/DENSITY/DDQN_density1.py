# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/12/17
...
V = np.random.uniform(15, 1, 1)                    # 人群移动速度
W = np.random.randint(0, 360, 1)                  # 人群移动角度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 4                                 # 无人机位置, 1个用户位置，2*2
        self.title('UAV测试环境-DDQN-1')
    ...
    def _build_maze(self):
        ...
        # --------------------------用户--------------------------------
        self.user_center = np.array([[580, 140]])
        for i in range(1):
    ...
    def reset_uav(self):
        ...
        return np.hstack((np.array([[self.canvas.coords(self.uav)[0], self.canvas.coords(self.uav)[1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 2)))

    def step(self, observation, action, t, U):
        ...
        s_ = np.hstack((np.array([[next_coords[0][0], next_coords[0][1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 2)))  # 1*32的数据， [[....]]
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s.reshape(1, 4), np.array([a]).reshape(1, 1), np.array([r/60]).reshape(1, 1), s_.reshape(1, 4)))
    ...
    def learn(self):
        ...
        # 给target网络输入s'以获得下一个最优a'，三维（1，50，25*1）
        ...
        q_next_eval = np.array(q_next_eval).reshape(50, 25)
...
def run_maze():
    ...
    for episode in range(N):
        ...
        while True:
            ...
            observation_, reward, Reward, done, u = env.step(observation, action, T, U)  # 状态更新，包括用户移动更新
