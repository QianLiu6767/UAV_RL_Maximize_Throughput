...
V = np.random.uniform(15, 1, 3)                # 人群移动速度
W = np.random.randint(0, 360, 3)                  # 人群移动角度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 8                                 # 无人机位置, 3个用户位置，4*2
        self.title('UAV测试环境-DDQN-3')
    ...
    def _build_maze(self):
        ...
        # --------------------------用户--------------------------------
        self.user_center = np.array([[380, 100], [100, 260], [280, 320]])
        for i in range(3):
    ...
    def reset_uav(self):
        ...
        return np.hstack((np.array([[self.canvas.coords(self.uav)[0], self.canvas.coords(self.uav)[1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 6)))
    ...
    def step(self, observation, action, FLAG, t, U):
        ...
        s_ = np.hstack((np.array([[next_coords[0][0], next_coords[0][1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 6)))  # 1*32的数据， [[....]]
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s.reshape(1, 8), np.array([a]).reshape(1, 1), np.array([r/60]).reshape(1, 1), s_.reshape(1, 8)))
    ...
    def learn(self):
        ...
        q_next_eval = np.array(q_next_eval).reshape(50, 75)
