...
V = np.random.uniform(1, 1, 5)                # 人群移动速度
W = np.random.randint(0, 360, 5)                  # 人群移动角度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 12                                 # 无人机位置, 15个用户位置，16*2
        self.title('UAV测试环境-DDQN-5')
    ...
    def _build_maze(self):
        ...
        # --------------------------用户--------------------------------
        self.user_center = np.array([[380, 100], [580, 140], [100, 260], [60, 540], [280, 320]])
        for i in range(5):
    ...
    def update_env(self, speed, angle):
        ...
        for i in range(5):
            ...
            speed[i] = 0.99 * speed[i] + (1 - 0.99) * 5.04 + (1 - 0.99) ** 2 * np.random.normal(1, 0.1, 1)  # 更新ith用户的速度
    ...
    def reset_uav(self):
        ...
        return np.hstack((np.array([[self.canvas.coords(self.uav)[0], self.canvas.coords(self.uav)[1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 10)))
    ...
    def step(self, observation, action, FLAG, t, U):
        ...
        s_ = np.hstack((np.array([[next_coords[0][0], next_coords[0][1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 10)))  # 1*32的数据， [[....]]
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s.reshape(1, 12), np.array([a]).reshape(1, 1), np.array([r/60]).reshape(1, 1), s_.reshape(1, 12)))
    ...
    def learn(self):
        ...
        # 给target网络输入s'以获得下一个最优a'，三维（1，50，125）
        ...
        q_next_eval = np.array(q_next_eval).reshape(50, 125)
