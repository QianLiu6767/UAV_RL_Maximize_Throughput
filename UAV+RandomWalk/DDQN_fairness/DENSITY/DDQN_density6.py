...
V = np.random.uniform(15, 1, 6)                # 人群移动速度
W = np.random.randint(0, 360, 6)                  # 人群移动角度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 14                                 # 无人机位置, 6个用户位置，7*2
        self.title('UAV测试环境-DDQN-6')
    ...
    def _build_maze(self):
        ...
        # --------------------------用户--------------------------------
        self.user_center = np.array([[380, 100], [180, 140], [100, 260], [280, 320], [140, 380], [520, 100]])
        for i in range(6):
    ...
    def reset_uav(self):
        ...
        return np.hstack((np.array([[self.canvas.coords(self.uav)[0], self.canvas.coords(self.uav)[1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 12)))
    ...
    def step(self, observation, action, FLAG, t, U):
        ...
        s_ = np.hstack((np.array([[next_coords[0][0], next_coords[0][1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 12)))  # 1*14的数据， [[....]]
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s.reshape(1, 14), np.array([a]).reshape(1, 1), np.array([r/60]).reshape(1, 1), s_.reshape(1, 14)))
    ...
    def learn(self):
        ...
        q_next_eval = np.array(q_next_eval).reshape(50, 150)
