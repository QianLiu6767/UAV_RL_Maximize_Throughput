...
V = np.random.uniform(15, 1, 7)                # 人群移动速度
W = np.random.randint(0, 360, 7)                  # 人群移动角度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 16                                 # 无人机位置, 7个用户位置，8*2
        self.title('UAV测试环境-DDQN-7')
    ...
    def _build_maze(self):
        ...
        # --------------------------用户--------------------------------
        self.user_center = np.array([[380, 100], [180, 140], [100, 260], [280, 320], [140, 380], [160, 440], [520, 100]])
        for i in range(7):
    ...
    def reset_uav(self):
        ...
        return np.hstack((np.array([[self.canvas.coords(self.uav)[0], self.canvas.coords(self.uav)[1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 14)))
    ...
    def step(self, observation, action, FLAG, t, U):
        ...
        s_ = np.hstack((np.array([[next_coords[0][0], next_coords[0][1]]]).reshape(1, 2), np.array([self.user_center]).reshape(1, 14)))  # 1*32的数据， [[....]]
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s.reshape(1, 16), np.array([a]).reshape(1, 1), np.array([r/60]).reshape(1, 1), s_.reshape(1, 16)))
    ...
    def learn(self):
        ...
        q_next_eval = np.array(q_next_eval).reshape(50, 175)
