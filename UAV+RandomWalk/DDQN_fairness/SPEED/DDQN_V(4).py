# !usr/bin/env python
# -*- coding:utf-8 -*-
# Author:LiuQian,time:2018/12/12
...
V = np.random.uniform(0.5, 0.5, 15)                # 人群移动速度
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.title('UAV测试环境-DDQN-(0.5, 0.5)-50m-15TU')
    ...
    def update_env(self, speed, angle):
        ...
        for i in range(15):
            ...
            speed[i] = 0.99 * speed[i] + (1 - 0.99) * 1.94 + (1 - 0.99) ** 2 * np.random.normal(1, 0.1, 1)  # 更新ith用户的速度
...
class DeepQNetwork:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        transition = np.hstack((s.reshape(1, 32), np.array([a]).reshape(1, 1), np.array([r/60]).reshape(1, 1), s_.reshape(1, 32)))
...
def run_maze():
    ...
    plt.plot(x1, r1, marker='o', label='DDQN-50m-15TU-(0.5, 0.5)', markersize=10)
    ...
    plt.plot(x1, y1, marker='o', label='DDQN-50m-15TU-(0.5, 0.5)', markersize=10)
