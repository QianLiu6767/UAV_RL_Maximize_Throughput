...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.n_features = 3  # 无人机位置, 用户位置，电池量
        self.battery = 10
        self.title('UAV测试环境')
    ...
    def step(self, action):
        ...
        for j in range(25):
            if next_coords == self.oval_center[j, :].tolist():
                # 飞行能耗
                ef = 0.1 * (np.sqrt(100 ** 2 + (s[0] - self.oval_center[j, 0]) ** 2 +
                                    (s[1] - self.oval_center[j, 1]) ** 2)) / 104
                ...
                s_ = np.hstack((np.array([next_coords[0] / (MAZE_H * UNIT), next_coords[1] / (MAZE_W * UNIT)]),
                                self.battery / 10))
                # self.user_center[o, 0] / (MAZE_W * UNIT), self.user_center[o, 1] / (MAZE_W * UNIT),
                print('Reward:', reward)
